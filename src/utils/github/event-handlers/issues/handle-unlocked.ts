import { IssuesUnlockedEvent } from "@octokit/webhooks-types";
...
export default async function githubHandleIssuesUnlockedEvent(data: IssuesUnlockedEvent) {
    ...
    originalMessages.map(async (originalMessage) => {
        await discordSendTextMessageToChannelWithRefference(originalMessage.tracker.discord_channel_id, `An issue was unlocked!\n\nCheck the changes - ${originalMessage.issue.url}`, originalMessage.id)
    })
    return
}