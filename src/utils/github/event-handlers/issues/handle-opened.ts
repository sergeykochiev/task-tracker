...
import { dbBulkGetTrackersByRepositoryOwnerAndName } from "../../../db/tracker";
...
export default async function githubHandleIssuesOpenedEvent(data: IssuesOpenedEvent) {
    const targetTrackers = await dbBulkGetTrackersByRepositoryOwnerAndName(data.repository.owner.login, data.repository.name)
