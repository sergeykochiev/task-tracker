...
export default async function githubHandleIssuesAssignedEvent(data: IssuesAssignedEvent) {
    ...
    originalMessages.map(async (originalMessage) => {
        await discordSendTextMessageToChannelWithRefference(originalMessage.tracker.discord_channel_id, `An issue was assigned a new user!\n\nCheck the changes - ${originalMessage.issue.url}`, originalMessage.id)
    })
    return
}