...
export default async function githubGetInstallationAccessToken(jwt: string, installId: string, repositoryIds?: string[]) {
    return await githubMakeRequest<Endpoints["POST /app/installations/{installation_id}/access_tokens"]["response"]["data"]>(GITHUB_ENDPOINTS.GET_ACCESS_TOKEN(installId), jwt, {
        method: "POST",
