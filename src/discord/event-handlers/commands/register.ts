...
export default async function discordHandleRegisterCommand(data: APIApplicationCommandInteraction) {
    ...
    if (!targetRepository.installationId) {
        ...
        if(res.hasInstall) {
            const installationId = String(res.res.id)
            await RepositoryEntity.update(targetRepository.id, { installationId })
        } else {
            trackerStatus = RegisterStatus.PendingInstallation
        }
    }
    await saveOrUpdateTracker(targetTracker, targetRepository, data.channel.id, trackerStatus, role)
